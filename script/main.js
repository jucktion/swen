...
let s=setSpeech();s.then((voices)=>{this.voices=this.synth.getVoices();});},speaker:function(){this.speak=this.$root.voice.map(x=>x.title);if(this.synth.speaking&&!this.synth.paused){this.synth.pause();this.state=0;this.playPause();return;}
if(this.synth.paused&&this.synth.speaking){this.synth.resume();this.state=1;this.playPause();return;}
...
utterThis.voice=this.voices[this.selectedVoice];utterThis.pitch=this.pitch;utterThis.rate=this.rate;this.state=1;this.synth.speak(utterThis);this.playPause();}},stopSpeak:function(){this.synth.cancel();this.state=0;this.stopShown=false;this.playPause();},playPause:function(){this.svg=this.state==0?'/img/play.svg':'/img/pause.svg';},onScroll:function(){this.showFab=(window.innerHeight+window.scrollY)!=document.body.offsetHeight;}}});let vm=new Vue({el:'#app',data(){return{voice:[]}}});