function getContent(t,e="GET",s){let i=new XMLHttpRequest;i.open(e,t,!0),i.send(),i.onreadystatechange=function(){4===i.readyState&&200===i.status&&(i.responseText,"function"==typeof s&&s.apply(i))}}let url=new URL(document.URL),rd=url.searchParams.get("r")?"https://"+url.searchParams.get("r"):"https://reddit.com",lang=null===url.searchParams.get("l")?navigator.language:""==url.searchParams.get("l")?navigator.language:url.searchParams.get("l").length>0?url.searchParams.get("l"):"",hash=""!=window.location.hash?window.location.hash.split("#")[1].split(","):"",h1=hash.length>=1?hash[0].toLowerCase():"",h2=hash.length>=2?hash[1].toLowerCase():"";Vue.component("tablet",{template:`
    <div>
...
<span @click="settingsShown = !settingsShown" class="close">x</span>
    ...
    `,data:()=>({settingsShown:!1,stopShown:!1,showFab:!1,svg:"/img/play.svg",speak:[],saythis:null,pitch:1,rate:1,separator:".... and in other news ....",synth:window.speechSynthesis,voices:[],selectedVoice:0,startItem:1,endItem:25}),mounted(){this.showFab="speechSynthesis"in window,this.showFab&&(this.loadVoices(),this.synth.cancel());window.matchMedia("(max-width: 512px)").matches||window.addEventListener("scroll",this.onScroll)},beforeDestroy(){window.removeEventListener("scroll",this.onScroll)},methods:{loadVoices:function(){new Promise(function(t,e){let s;s=setInterval(()=>{0!==window.speechSynthesis.getVoices().length&&(t(window.speechSynthesis.getVoices()),clearInterval(s))},10)}).then(t=>{this.voices=lg?t.filter(function(t){return -1!=t.lang.indexOf(lang)}):t})},speaker:function(){if(null!=this.saythis||this.synth.speaking||(this.speak=this.$root.voice.map(t=>t.title),this.saythis=this.speak.slice(this.startItem-1,this.endItem).join(this.separator)),this.synth.speaking&&!this.synth.paused){this.synth.pause(),this.playPause(0);return}if(this.synth.paused&&this.synth.speaking){this.synth.resume(),this.playPause(1);return}if(!this.synth.paused&&null!=this.saythis){let t=new SpeechSynthesisUtterance(this.saythis);this.stopShown=!0,now=this,t.onend=function(t){now.stopSpeak()},t.onerror=function(t){console.error("SpeechSynthesisUtterance.onerror")},t.voice=this.voices[this.selectedVoice],t.pitch=this.pitch,t.rate=this.rate,this.synth.speak(t),this.playPause(1)}},stopSpeak:function(){this.synth.cancel(),this.stopShown=!1,this.saythis=null,this.playPause(0)},playPause:function(t){this.svg=0==t?"/img/play.svg":"/img/pause.svg"},onScroll:function(){this.showFab=window.innerHeight+window.scrollY!=document.body.offsetHeight}}});let vm=new Vue({el:"#app",data:()=>({voice:[]})});